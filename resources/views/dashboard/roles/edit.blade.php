...
@section('content')
    <div class="container mx-auto">
        <div class="bg-white rounded p-4 shadow m-4">
            <h1 class="text-2xl uppercase">Update A Role</h1>

            <form action="{{ route('dashboard.role.update', $role->id) }}" class="mt-4" method="post"
                ...
                <div>
                    ...
                    <input type="text" class="form-control mt-2 @error('name') is-invalid @enderror"
                           placeholder="Name..." value="{{ old('name') ?? $role->name }}" id="name" name="name"
                           autofocus/>
                        ...
                        <select type="text" class="@error('permissions') is-invalid @enderror"
                            ...
                            @foreach($permissions as $permission)
                                <option value="{{ $permission->id }}"
                                    {{ $role->hasPermissionTo($permission) ? 'selected' : '' }}>{{ $permission->name }}</option>
                            @endforeach
                        </select>
                    </div>
